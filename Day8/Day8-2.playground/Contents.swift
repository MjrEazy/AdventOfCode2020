...
final class Instruction: CustomStringConvertible, NSCopying {
    var instr: String
    let oper: Int
    ...
    func copy(with zone: NSZone? = nil) -> Any {
        let copy = Instruction(string: instr + " " + String(oper))
        return copy
    }
}
...
//    let input = try String(contentsOfFile: "/Users/David/Documents/AdventOfCode2020/AdventOfCode2020/Day8/Day8_input_test.txt", encoding: String.Encoding.utf8)
    ...
    for i in stride(from:dodgyCode.count-1, through: 0, by: -1) {
        // urgh! brute force it by trying all combinations, taking a punt it's nearer the end than the beginning and even worse...
        // use a deep copy to keep original code each time, there has to be a better way to do this...
        var attemptedFix = dodgyCode.deepCopy()
